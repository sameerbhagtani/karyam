...
function getAssetUrl(asset: unknown): string {
  ...
  if (typeof asset === 'object' && 'src' in asset && typeof (asset as { src: string }).src === 'string') {
    return (asset as { src: string }).src
  }
  return ''
}

function AnimatedMark({ className = '' }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 240 240"
...
export default function LandingLoader() {
  ...
  const [phase, setPhase] = useState<'loading' | 'morphing'>('loading')
